...
int main(int argc, char **argv)
{
    ...
    ex.f1 = al_load_olf_font(font_file, 48, 0);
    ex.f2 = al_load_olf_font(font_file, 48, ALLEGRO_OLF_NO_KERNING);
    ex.f3 = al_load_olf_font(font_file, 12, 0);
    /* Specifying negative values means we specify the glyph height
     * in pixels, not the usual font size.
     */
    ex.f4 = al_load_olf_font(font_file, -140, 0);
    ex.f5 = al_load_olf_font(font_file, 12, ALLEGRO_OLF_MONOCHROME);
